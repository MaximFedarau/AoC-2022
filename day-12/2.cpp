...
// ! TASK 2 is very similar to TASK 1, because in TASK 1 we go from the beginning to the end, but in TASK 2 we go from the end to the closest beginning
...
int main() {
    ...
    Node start_node = {end.at(0), end.at(1), 0};
    ...
    while (true) {
        ...
        // if our current node is an 'a' or 'S', then we just std::cout our distance
        if (field.at(current_node_x).at(current_node_y) == 1) {
        ...
        for (std::vector<int64_t> coord: possible_coords) {
            ...
            // checking heights
            if (field.at(current_node_x).at(current_node_y) - field.at(new_node.x).at(new_node.y) <= 1) nodes.insert(nodes.begin(), new_node); // place neighbour in the beginning of the vector, because this action allows us to choose the closest to current_node node with the minimum distance - important!
        } 
    }
}
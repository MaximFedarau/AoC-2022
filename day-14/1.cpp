...
int main() {
    ...
    int64_t counter = 0;
    while (true) {
        std::pair<int64_t, int64_t> sand_position = std::make_pair(0, SAND_SOURCE); // one sand unit coords
        while (true) {
            // if a sand unit reaches the bottom, then std::cout our result and stop program
            if (sand_position.first == FIELD_SIZE - 1) {

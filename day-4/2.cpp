...
int main() {
    ...
    while (getline(std::cin, input)) {
        ...
        if ((c >= a && d <= b  && c <= b) || (a >= c && a <= d && b <= d)) counter++; // if one pair fully contains the other (snippet from the first task)
        else if ((c <= b && d >= b) || (a <= d && b >= d)) counter++; // if there is an overlap, but no containing pair

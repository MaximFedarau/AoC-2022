...
struct Monkey {
    ...
    int64_t test_number;
    std::vector<int64_t> monkeys_list;
};
...
int main() {
    ...
    std::vector<int64_t> counters_array(monkeys.size(), 0);
    ...
    for (int64_t i = 0; i < 20; ++i) { // 20 rounds
        for (size_t j = 0; j != monkeys.size(); ++j) {
            ...
            for (size_t x = 0; x != (*original_monkey).items.size(); ++x) {
                int64_t item = (*original_monkey).items.at(x), operation_member = (*original_monkey).operation.substr(6) == "old" ? item : stoi((*original_monkey).operation.substr(6));
                ...
                int64_t test_number = (*original_monkey).test_number;

                item %= general_remainder;
